...
{
    public abstract class FundingSchema11TestBase
    {
        private const string ResourceFileRoot = "CalculateFunding.Generators.Schema11.UnitTests.Resources";
        private static readonly Assembly Assembly = typeof(FundingSchema11TestBase).Assembly;
        ...
        [TestInitialize]
        public async Task JsonSchema11TestBaseInitialise()
        {
            FundingSchema = await JsonSchema.FromJsonAsync(GetEmbeddedFileContents("funding-schema1.1.json"));
        }
        ...
        protected void ThenTheJsonValidatesAgainstThe1_1FundingSchema(string json)
        {
            ...
            Assert.Fail($"Resulting json did not validate against the 1.1 funding schema;\n{validationErrors}");
        }
    }
}