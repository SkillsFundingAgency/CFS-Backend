Imports System
Imports Common
Imports Common.Attributes

Public Class RecoupmentNewISBDataset

    Public Shared DatasetDefinitionName As String = "Recoupment New ISB"

    Public Property Id As String

    Public Property BudgetId As String

    Public Property ProviderUrn As String

    Public Property ProviderName As String

    Public Property DatasetName As String

    Public Property BasicEntitlementPrimary As String

    Public Property BasicEntitlementKS3 As String

    Public Property BasicEntitlementKS4 As String

    Public Property FreeSchoolMealsPrimary As String

    Public Property FreeSchoolMealsSecondary As String

    Public Property IDACIP0 As String

    Public Property IDACIP1 As String

    Public Property IDACIP2 As String

    Public Property IDACIP3 As String

    Public Property IDACIP4 As String

    Public Property IDACIP5 As String

    Public Property IDACIP6 As String

    Public Property IDACIS0 As String

    Public Property IDACIS1 As String

    Public Property IDACIS2 As String

    Public Property IDACIS3 As String

    Public Property IDACIS4 As String

    Public Property IDACIS5 As String

    Public Property IDACIS6 As String

    Public Property EALP As String

    Public Property EALS As String

    Public Property LAC As String

    Public Property LowAttainmentP As String

    Public Property LowAttainmentS As String

    Public Property MobilityP As String

    Public Property MobilityS As String

    Public Property LumpSum As String

    Public Property SparsityFunding As String

    Public Property LondonFringe As String

    Public Property SplitSites As String

    Public Property Rates As String

    Public Property PFI As String

    Public Property HistoricalCommitmentsof6thFormFundingfromDSG As String

    Public Property _1617ApprovedExceptionalCircumstance3 As String

    Public Property _1617ApprovedExceptionalCircumstance4 As String

    Public Property _1617ApprovedExceptionalCircumstance5 As String

    Public Property _1617ApprovedExceptionalCircumstance6 As String

    Public Property BasicEntitlementTotal As String

    Public Property TotalAllocation As String

    Public Property _1617ApprovedExceptionalCircumstance2Reservedforadditionalsparsitylumpsum As String

    Public Property AENTotal As String

    Public Property SchoolFactorstotal As String

    Public Property NotionalSENBudget As String

    Public Property PrimaryFunding As String

    Public Property SecondaryFunding As String

    Public Property _1617MFGBudget As String

    Public Property _1617MFGUnitValue As String

    Public Property _1516MFGUnitValue As String

    Public Property MFGchange As String

    Public Property MFGValueadjustment As String

    Public Property _1617PostMFGBudget As String

    Public Property _1617PostMFGperpupilBudget As String

    Public Property YearonyearChange As String

    Public Property Dedelegation As String

    Public Property PostDedelegationbudget As String

    Public Property _1617MFGAdjustment As String

    Public Property _1617ApprovedExceptionalCircumstance1ReservedforAdditionallumpsumforschoolsamalgamatedduringFY1516 As String
End Class
