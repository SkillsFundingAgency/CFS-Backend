    ...
    public class BuildProjectsService : JobProcessingService, IBuildProjectsService, IHealthChecker
    {
        ...
        public override async Task Process(Message message)
        {
            ...
            if (summariesExist)
            {
                ...
                {
                    // for FDZ if the provider version cached is not equal to the one set against specification then we need to refresh cache
                    string cachedProviderVersion = await _cacheProvider.GetAsync<string>($"{CacheKeys.ScopedProviderProviderVersion}{specificationId}");

                    refreshCachedScopedProviders = cachedProviderVersion != specificationSummary.ProviderVersionId;
                }
            }
