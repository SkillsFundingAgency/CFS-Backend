...
{
    public static class OnCalcsGenerateAllocationResultsTimerFired
    {
        [FunctionName("on-calcs-generate-allocations-timer-fired")]
        public static async Task Run([TimerTrigger("0 */5 * * * *")]TimerInfo myTimer)
        {
            using (var scope = IocConfig.Build().CreateScope())
            {
                ...
                await messagePump.ReceiveAsync("calc-events", "calc-events-generate-allocations-results",
                         async message =>
                         {
